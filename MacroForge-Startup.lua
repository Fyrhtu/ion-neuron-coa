...
function MacroForge:EnsureButtonSpecData(buttonDB, specIndex, state)
...
function MacroForge:GetButtonSkeleton(barClass)
...
function MacroForge:MergeButtonConfigDefaults(barClass, config)
	local buttonSkeleton = MacroForge:GetButtonSkeleton(barClass)
...
function MacroForge:CreateButtonDatabaseEntry(barClass)
	local buttonSkeleton = MacroForge:GetButtonSkeleton(barClass)
...
function MacroForge:CreateBarDatabaseEntry(barClass)
	...
	entry.buttons = {}
	if barClass == "ActionBar" then
		entry.buttons[1] = MacroForge:CreateButtonDatabaseEntry(barClass)
	end
	return entry
end
...
--- but we have them scattered between macroforge-defaults and macroforge-db-defaults
function MacroForge:InitializeEmptyDatabase(DB)
	DB.firstRun = false
	...
	for barClass, registeredData in pairs(MacroForge.registeredBarData) do
...
function MacroForge:CreateBarsAndButtons(profileData)
	-- remove blizzard controlled bars from the list of bars we will create
	-- but still keep MacroForge action bars regardless
	local macroForgeBars =
		Array.filter(
			function (barPair)
				local bar, _ = unpack(barPair)
			  return not profileData.blizzBars[bar] or bar == "ActionBar"
			end,
		Array.fromIterator(pairs(MacroForge.registeredBarData)))

	-- make the frames for the bars now
	for _, barData in pairs (macroForgeBars) do
		local barClass, barClassData = unpack(barData)
		for id,data in pairs(barClassData.barDB) do
			if data ~= nil then
				local newBar = MacroForge.Bar.new(barClass, id) --this calls the bar constructor
