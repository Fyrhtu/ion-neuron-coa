...
function MacroForge:HideBlizzardUI(profileDatabase)
	...
	if not blizzBars.ActionBar then
		...
		if not MacroForge:IsHooked("MultiActionBar_ShowAllGrids") then
			MacroForge:RawHook("MultiActionBar_ShowAllGrids", function() end, true)
		end
		if not MacroForge:IsHooked("MultiActionBar_HideAllGrids") then
			MacroForge:RawHook("MultiActionBar_HideAllGrids", function() end, true)
		end
	end

	-- Cast/Mirror/XP/Rep and bag/menu/exit/extra/zone are no longer managed by MacroForge.
...
function MacroForge:ToggleBlizzUI(blizzBars)
