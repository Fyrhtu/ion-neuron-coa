...
function NeuronGUI:GenerateIconList()
	...
	if Neuron.UpdateSpellCache then
		Neuron:UpdateSpellCache()
	end

	for _, v in pairs(Neuron.spellCache or {}) do
		addUniqueIcon(seen, v.icon)
		addUniqueIcon(seen, v.altIcon)
	end

	for _, v in pairs(Neuron.itemCache or {}) do
		...
		if texture then
			...
			iconFrame:SetCallback("OnClick", function()
				if Neuron.currentButton then
					Neuron.currentButton:SetMacroIcon(texture)
				...
				NeuronGUI:RefreshEditor("button")
			end)
