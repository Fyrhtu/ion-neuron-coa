...
-- these panels are then loaded via MacroForgeGUI:LoadInterfaceOptions

local function profileOptions()
	local options = LibStub("AceDBOptions-3.0"):GetOptionsTable(MacroForge.db)

	--enhance the database object with per spec profile features
	if MacroForge.isWoWRetail or MacroForge.isWoWWotLK then
		local LibDualSpec = LibStub('LibDualSpec-1.0', true)
		if LibDualSpec then
			LibDualSpec:EnhanceDatabase(MacroForge.db, addonName)
			LibDualSpec:EnhanceOptions(options, MacroForge.db)
		end
	end
	return options
end
			...
			importexport={
				...
				args={
					...
					TextBox = {
						...
						validate = false,
						set = function(self, input) MacroForge:SetSerializedAndCompressedProfile(input) end,
						get = function() return MacroForge:GetSerializedAndCompressedProfile() end,
						width = "full",
					},
				},
			},
...
local function guiOptions()
	...
	local args = {
		...
		ApplyButton = {
			...
			func = function()
				MacroForge:ToggleBlizzUI(changes)
				ReloadUI()
			end
		},
	}
	for bar, _ in pairs(changes) do
		local reg = MacroForge.registeredBarData[bar]
		if not reg then
			changes[bar] = nil
		else
		args[bar] = {
	...
	args.MacroForgeMinimapButton = {
		order = 0,
		name = L["Display Minimap Button"],
		desc = L["Toggles the minimap button."],
		type = "toggle",
		set =  function() MacroForge:Minimap_ToggleIcon() end,
		get = function() return not DB.MacroForgeIcon.hide end,
		width = "full"
	}
	args.MacroForgeOverrides = {
...
function MacroForgeGUI:LoadInterfaceOptions()
