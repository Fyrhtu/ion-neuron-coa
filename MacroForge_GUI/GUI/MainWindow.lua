...
function MacroForgeGUI:RefreshEditor(defaultTab)
	addonTable.MacroForgeEditor:ReleaseChildren()
	...
	MacroForgeGUI:PopulateEditorWindow()

	if MacroForge.currentBar then
		addonTable.MacroForgeEditor:SetStatusText("|cffffd200" .. MacroForge.currentBar:GetBarName().."|cFFFFFFFF is currently selected. Left-click a different bar to change your selection.")
	else
		addonTable.MacroForgeEditor:SetStatusText("|cFFFFFFFFWelcome to the MacroForge editor, please select a bar to begin")
	end
end


function MacroForgeGUI:CreateEditor(defaultTab)
	addonTable.MacroForgeEditor = AceGUI:Create("Frame") --add it to our base addon table to reference later

	addonTable.MacroForgeEditor:SetTitle("MacroForge Editor")
	addonTable.MacroForgeEditor:EnableResize(true)
	if addonTable.MacroForgeEditor.frame.SetResizeBounds then -- WoW 10.0
		addonTable.MacroForgeEditor.frame:SetResizeBounds(EDITOR_MIN_WIDTH, EDITOR_MIN_HEIGHT)
	else
		addonTable.MacroForgeEditor.frame:SetMinResize(EDITOR_MIN_WIDTH, EDITOR_MIN_HEIGHT)
	end
	applyEditorSize(addonTable.MacroForgeEditor)
	if MacroForge.currentBar then
		addonTable.MacroForgeEditor:SetStatusText("|cffffd200" .. MacroForge.currentBar:GetBarName().."|cFFFFFFFF is currently selected. Left-click a different bar to change your selection.")
	else
		addonTable.MacroForgeEditor:SetStatusText("|cFFFFFFFFWelcome to the MacroForge editor, please select a bar to begin")
	end
	addonTable.MacroForgeEditor:SetCallback("OnClose", function() MacroForgeGUI:DestroyEditor() end)
	addonTable.MacroForgeEditor:SetLayout("Fill")

	-- make the thing closable with escape
	_G.MacroForgeEditorMainFrame = addonTable.MacroForgeEditor
	tinsert(UISpecialFrames, "MacroForgeEditorMainFrame")
...
function MacroForgeGUI:DestroyEditor()
	if addonTable.MacroForgeEditor then
		AceGUI:Release(addonTable.MacroForgeEditor)
		addonTable.MacroForgeEditor = nil
	end

	if MacroForge.barEditMode and addonTable.overlay and addonTable.overlay.BarEditor then
		addonTable.overlay.BarEditor.syncAll()
	end
end

function MacroForgeGUI:PopulateEditorWindow()
	...
	tabFrame:SetCallback("OnGroupSelected", function(frame, _, value) MacroForgeGUI:SelectTab(frame, _, value) end)
	addonTable.MacroForgeEditor:AddChild(tabFrame)
	tabFrame:SelectTab(currentTab)
	-- AceGUI TabGroup does not always fire OnGroupSelected on programmatic SelectTab (3.3.5).
	MacroForgeGUI:SelectTab(tabFrame, nil, currentTab)
end


function MacroForgeGUI:SelectTab(tabFrame, _, value)
	tabFrame:ReleaseChildren()
	if value == "bar" then
		MacroForgeGUI:BarEditPanel(tabFrame)
	elseif value == "button" then
		...
		MacroForgeGUI:ButtonsEditPanel(tabFrame)
	else
