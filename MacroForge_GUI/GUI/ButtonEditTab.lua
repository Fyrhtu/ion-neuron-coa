...
local function refreshIconPreview(frame, data)
	local texture = MacroForge.currentButton:GetAppearance(data)
	if texture then
...
local function visibilityMatchesBarState(barState, visibilityState)
	local managed = MacroForge.MANAGED_BAR_STATES[barState]
...
local function getStateList()
	local barData = MacroForge.currentButton.bar.data

	local barStates =
		Array.filter(function(state) return barData[state] end,
		Array.map(function(state) return state[1] end,
		Array.fromIterator(pairs(MacroForge.MANAGED_BAR_STATES))))
	...
	if barData.stance then
		...
		local stanceInfo = MacroForge.MANAGED_BAR_STATES.stance
		local rangeStop = stanceInfo and stanceInfo.rangeStop or 8
		for slot = 1, rangeStop do
			local stateKey = "stance" .. slot
			if not seen[stateKey] and MacroForge.STATES[stateKey] then
...
local function getHomeStateLabel()
	if MacroForge.currentButton.bar.data.stance then
		local homestate = MacroForge.MANAGED_BAR_STATES.stance and MacroForge.MANAGED_BAR_STATES.stance.homestate
		if homestate and MacroForge.STATES[homestate] then
			return MacroForge.STATES[homestate]
		end
	end
	return MacroForge.STATES.homestate
end
...
local function buttonEditPanel(specData, update)
	...
	previewIconFrame:SetCallback("OnClick", function() MacroForgeGUI:IconFrame_OnClick() end)
	...
	clearButton:SetCallback("OnClick", function()
		local reset = CopyTable(MacroForge.ActionButton.EMPTY_STATE_DATA)
		update(reset)
...
function MacroForgeGUI:ButtonsEditPanel(topContainer)
	MacroForge:ToggleButtonEditMode(true)

	if not MacroForge.currentButton then
		return
	end

	local multiSpec = MacroForge.currentButton.bar:GetMultiSpec()
	...
	if multiSpec then
		...
		for specIndex, specName in ipairs(specs) do
			MacroForge:EnsureButtonSpecData(MacroForge.currentButton.DB, specIndex, "homestate")
			...
			for _, state in ipairs(getStateList()) do
				children[#children + 1] = {
					value = state,
					text = MacroForge.STATES[state] or state,
				}
			end
		...
		MacroForge:EnsureButtonSpecData(MacroForge.currentButton.DB, 1, "homestate")
		local children = Array.map(
			function(state)
				return { value = state, text = MacroForge.STATES[state] or state }
			end,
			getStateList()
		)
		treeNodes[1] = {
	...
	specButtonTree:SetCallback("OnGroupSelected", function(container, _, joinedState)
		...
		local _, stateData = MacroForge:EnsureButtonSpecData(MacroForge.currentButton.DB, specIndex, state)

		MacroForgeGUI.editingSpecIndex = specIndex
		MacroForgeGUI.editingState = state
		MacroForgeGUI.editingStateData = stateData
		...
		local buttonEditor = buttonEditPanel(stateData, function(data)
			...
			if not InCombatLockdown() then
				MacroForge.currentButton:LoadDataFromDatabase(specIndex, state)
				if MacroForge.currentButton.UpdateButtonSpec then
					MacroForge.currentButton:UpdateButtonSpec(specIndex)
				else
					MacroForge.currentButton.bar:Load()
				end
			end
		end)
	...
	if multiSpec and not MacroForgeGUI.editingStateData then
