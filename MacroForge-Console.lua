...
function MacroForge:slashHandler(input)
	if string.len(input)==0 or input:lower() == "help" then
		MacroForge:printSlashHelp()
		return
	end
	...
	if command == "config" and MacroForge.barEditMode == false then
		args[1] = true
	end
	if command == "edit" and MacroForge.buttonEditMode == false then
		args[1] = true
	end
	if command == "bind" and MacroForge.bindingMode == false then
		args[1] = true
	end
	...
	for i = 1,#slashFunctions do
		...
		if command == slashFunctions[i][1]:lower() then
			...
			if func == "ChangeBar" then --intercept our bar assignment and reassign to the new bar, if it exists
				local newBar
				for _,v in pairs(MacroForge.BarIndex) do
			...
			if MacroForge[func] then
				MacroForge[func](MacroForge, args[1])
			elseif bar and bar[func] then
				...
				MacroForge:Print(L["No bar selected or command invalid"])
			end
			return
		end
	end
...
function MacroForge:printSlashHelp()

	MacroForge:Print("---------------------------------------------------")
	MacroForge:Print(L["How to use"]..":   ".."/macroforge".." <"..L["Command"]:lower().."> <"..L["Option"]:lower()..">")
	MacroForge:Print(L["Command List"]..":")
	MacroForge:Print("---------------------------------------------------")
...
function MacroForge:PrintStateList()
	...
	local count = 1
	for _,v in pairs(MacroForge.MANAGED_BAR_STATES) do
	...
	MacroForge:Print(list..L["Custom_Option"])
end

function MacroForge:PrintBarTypes()
	MacroForge:Print("---------------------------------------------------")
	MacroForge:Print("     "..L["How to use"]..":   ".."/macroforge".." "..L["Create"]:lower().." <"..L["Option"]:lower()..">")
	MacroForge:Print("---------------------------------------------------")

	for k,v in pairs(MacroForge.registeredBarData) do
		MacroForge:Print("    |cff00ff00"..k..":|r "..v.barLabel)
	end
end
